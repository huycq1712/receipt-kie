...
class Document:
    def __init__(self, boxes_and_transcripts_file: Path, image_file: Path,
                 ...
                 image_index=None, boxes_and_transcripts_list=None, image=None):
        '''
        ...
        try:
            ...
            if self.training:
                ...
                if boxes_and_transcripts_list is not None:
                    boxes_and_transcripts_data = []
                    for index, text_box in enumerate(boxes_and_transcripts_list):
                        transcript = text_box['text']
                        points = np.array(text_box['box']).reshape(-1).tolist()
                        boxes_and_transcripts_data.append((index, points, transcript))
                else:
            ...
            # read image
            image = np.array(image) #if type(image) is np.ndarray else image
            print(image.shape)
            #image = cv2.imread(image_file.as_posix()) if image is None else image
